      ...
      try {
        ...
        if (isNumericId && orderNoParam) {
          ...
          try {
            ...
            throw new Error(
              `Failed to fetch order details: ${orderError instanceof Error ? orderError.message : "Unknown error"}`,
            );
          }
        }

        const response = await invoiceApi.getInvoice(
          userId,
          actualOrderNo,
          sessionKey,
        );
        ...
        if (response.status === true && response.data) {
          ...
          if (Array.isArray(response.data)) {
            if (response.data.length === 0) {
              ...
              const errorMsg =
                response.message ||
                "Invoice not found. Please check the order number.";
              setError(errorMsg);
          ...
          const errorMsg =
            response?.message ||
            `Failed to load invoice. Status: ${response?.status}, Has data: ${!!response?.data}`;
          setError(errorMsg);
          toast.error(errorMsg);
        }
      } catch (err) {
