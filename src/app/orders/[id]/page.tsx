...
const OrderDetailPage = () => {
  ...
  const [reviewStatus, setReviewStatus] = useState<
    "arriving" | "cancelled" | "delivered" | null
  >(null);

  // Helper to map new status to review status format
  const mapToReviewStatus = (
    status: string,
  ): "arriving" | "cancelled" | "delivered" => {
    ...
    if (
      ...
      statusUpper === "ORDER_RETURN" ||
      statusUpper === "NOT_DELIVERED"
    ) {
      return "cancelled";
    }
    return "arriving";
  ...
  const handleDownloadInvoice = () => {
    if (!orderId) {
      toast.error("Unable to view invoice. Please try again.");
      return;
    }
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      if (rawOrderId && sanitizeOrderId(rawOrderId) !== rawOrderId) {
        logger.warn(
          "Order ID was sanitized",
          { original: rawOrderId, sanitized: orderId },
          "orders/[id]/page",
        );
      }
      ...
      try {
        ...
        const response = await orderDetailsApi.getOrderDetails(
          orderId,
          user?.id?.toString() || "",
          sessionKey,
        );
        ...
        if (response && response.status && response.data) {
          ...
          if (firstItem) {
            ...
            const localOrder: Order = {
              ...
              productImage:
                data.order_details.thumbnail || firstItem.image || "",
        ...
        if (err instanceof Error) {
          if (
            ...
            setError(
              `Order not found. The order "${orderId}" may not exist or you may not have permission to view it.`,
            );
          } else if (
            ...
            setError(
              "Server error: The backend is returning an error page instead of order data. Please try again later or contact support.",
            );
      ...
      <div className="container mx-auto px-4 py-8 space-y-10">
        <div className="flex items-center justify-between mb-4">
          <Button
            variant="default"
            onClick={() => router.back()}
            className="text-black font-normal hover:underline hover:cursor-pointer"
          >
            ← Back To Orders
          </Button>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 border-b md:pb-6 pb-4">
          <div className="flex gap-4 flex-1">
            ...
            <div className="flex-1 flex flex-col md:text-[18px] text-[16px]">
              ...
              <h2 className="font-[Grafiels] md:text-[18px] text-[13px] md:line-clamp-2 line-clamp-1 leading-tight md:mb-1 text-[#1A2819]">
                {order.productName}
              </h2>
              <p className="text-gray-600 md:text-[15px] text-[12px] md:mb-1 mb-0">
                Quantity: {order.quantity}
              </p>
              <p className="text-gray-600 md:text-[15px] text-[13px] md:max-w-[70%] md:line-clamp-2 line-clamp-1">
                {order.shortdecs}
              </p>
            </div>
          </div>
