...
export async function apiFetch<T = unknown>(
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        const timeoutError = new Error(
          `Request timeout: API call to ${endpoint} took too long (${API_CONFIG.TIMEOUT}ms timeout)`,
        );

        if (
          attempt < maxAttempts - 1 &&
          shouldRetry(timeoutError, attempt, retryConfig)
        ) {
          const delay = calculateRetryDelay(attempt, retryConfig?.baseDelay);
          await sleep(delay);
      ...
      if (error instanceof TypeError && error.message === "Failed to fetch") {
        ...
        if (
          attempt < maxAttempts - 1 &&
          shouldRetry(networkError, attempt, retryConfig)
        ) {
          const delay = calculateRetryDelay(attempt, retryConfig?.baseDelay);
          await sleep(delay);
  ...
  throw (
    lastError ||
    new Error(`Failed to fetch ${endpoint} after ${maxAttempts} attempts`)
  );
}
