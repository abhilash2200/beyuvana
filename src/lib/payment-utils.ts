...
import type { PaymentApiResponse, PaymentResponseData } from "./types/payment";
import { extractRedirectUrl, isPaymentResponseData } from "./types/payment";
...
const getRedirectFromObject = (obj: Record<string, unknown>): string | null => {
  ...
  const nestedRedirect =
    getNestedProperty<string>(obj, ["data", "redirect_url"], isString) ||
    getNestedProperty<string>(obj, ["data", "redirectUrl"], isString) ||
    getNestedProperty<string>(obj, ["data", "redirect_path"], isString) ||
    getNestedProperty<string>(obj, ["data", "redirectPath"], isString);

  return nestedRedirect || null;
};

export function getRedirectPathFromStatus(
...
export function parseNonJsonResponse(responseText: string): PaymentApiResponse {
  ...
  throw new Error(
    "Backend did not provide redirect URL. Please check your orders page.",
  );
}
...
export async function callPaymentResponseAPI(
  ...
  const endpoints = [`/api/payment_response/?${queryParams.toString()}`];

  for (const endpoint of endpoints) {
    try {
      const response = await apiFetch<PaymentResponseData>(endpoint, {
        method: "GET",
      });
      return {
