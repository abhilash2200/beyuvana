...
export function extractErrorInfo(error: unknown): AppError {
    ...
    if (typeof error === "object" && error !== null) {
        ...
        const message = String(errorObj.message || errorObj.error || "An unknown error occurred");
 ...
 */
export function getUserFriendlyMessage(error: AppError, customMessage?: string): string {
    if (customMessage) {
...
export function handleError(
    ...
    // Log the error
    if (!silent) {
        logger[logLevel](`Error in ${context}`, appError.originalError || appError.message, context);
    }
...
function showErrorNotification(error: AppError, customMessage?: string): void {
    const message = customMessage || error.userMessage || getUserFriendlyMessage(error);
...
export function createErrorHandler(context: string) {
    return (error: unknown, options?: { userMessage?: string; showToast?: boolean; silent?: boolean }) => {
        return handleError(error, {
            context,
            ...options,
        });
