...
const ENV_VAR_CONFIGS: EnvVarConfig[] = [
    ...
    },
    {
        ...
        description: "Proxy URL for API requests (used when API_BASE_URL is not set)",
        validator: (value) => {
            if (!value.startsWith("/")) {
    ...
    },
    {
        ...
        validator: (value) => {
            if (!["development", "production", "test"].includes(value)) {
                return { valid: false, error: "NODE_ENV must be 'development', 'production', or 'test'" };
            }
            return { valid: true };
        },
    },
...
export function getEnvironmentSummary(): string {
    ...
    if (result.summary.optional.missing.length > 0) {
        result.summary.optional.missing.forEach((name) => {
            ...
            const defaultValue = config?.defaultValue ? ` (default: ${config.defaultValue})` : "";
