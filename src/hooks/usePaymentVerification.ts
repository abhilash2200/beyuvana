...
export function usePaymentVerification(
  ...
  const defaultNoPaymentMessage =
    "No payment information found. Please check your orders or contact support.";
  const noPaymentInfoMessage =
    options.noPaymentInfoMessage || defaultNoPaymentMessage;
  ...
  const verifyPaymentStatus = useCallback(
    async (storedOrderId: string) => {
      try {
        ...
        const result =
          response.data || (isPaymentResponseData(response) ? response : null);
        ...
        setStatus("error");
        setErrorMessage(
          "Unable to verify payment status. Please refresh or contact support.",
        );
      } finally {
  ...
  useEffect(() => {
    ...
    const processPaymentCallback = async () => {
      ...
      try {
        ...
        setStatus("error");
        setErrorMessage(
          "Failed to process payment callback. Please try again or contact support.",
        );
      }
    };
