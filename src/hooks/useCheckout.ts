    ...
    async (
      ...
      try {
        ...
        if (checkoutCartItems.length === 0) {
          toast.error(
            "No valid items in cart. Please add items and try again.",
          );
        ...
        const checkoutRequest: CheckoutRequest = {
          ...
          payment_info: {
            pay_gateway_name:
              selectedPayment === PAYMENT_METHODS.PREPAID ? "phonepe" : "cod",
            pay_status: "",
            txn_id: "",
          },
          type: "web",
        ...
        if (response.status || response.success) {
          if (selectedPayment === PAYMENT_METHODS.PREPAID && response.data) {
            ...
            if (Array.isArray(paymentResponse) && paymentResponse.length > 0) {
              ...
              paymentResponseItem = paymentResponse as {
                orderId?: string;
                redirectUrl?: string;
              };
            }
            ...
            const redirectUrl =
              paymentResponseItem?.redirectUrl ||
              data.order_details?.payment_redirect_url ||
              null;
            const orderId =
              paymentResponseItem?.orderId || data.order_details?.id || null;
