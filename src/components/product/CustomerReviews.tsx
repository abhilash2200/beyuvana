  ...
  ({
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        try {
          const filterParams: ProductsListRequest = {
            filter: {},
            page: 1,
            limit: 10,
          };
          if (productName)
            (filterParams.filter as Record<string, unknown>).product_name = [
              productName,
            ];
          if (designSlug)
            (filterParams.filter as Record<string, unknown>).categorykey = [
              designSlug,
            ];
          const filteredResp = await productsApi.getList({
            ...(filterParams as unknown as {
              filter?: { product_name?: string[]; categorykey?: string[] };
              page?: number;
              limit?: number;
            }),
          });
          ...
          if (!Array.isArray(list) || list.length === 0) {
            const searchResp = await productsApi.getList({
              searchTerms: productName || designSlug || "",
              page: 1,
              limit: 50,
            });
          ...
          const byExactName = list.find(
            (p: Product) =>
              (
                p as Product & { product_name?: string }
              )?.product_name?.toLowerCase?.() === nameLower,
          );
          ...
          if ((candidate as Product | undefined)?.id) {
            const parsed = parseInt(
              (candidate as Product).id as unknown as string,
              10,
            );
        ...
        try {
          ...
          const maybeObj =
            (rawData as { reviews?: unknown; data?: unknown }) || {};
          ...
          const normalized: ReviewItem[] = arr.map((item, idx) => {
            ...
            const created =
              (item.created_on as string) ||
              (item.created_at as string) ||
              undefined;
            const ratingRaw = item.star_ratting as
              | string
              | number
              | null
              | undefined;
    ...
    const content = useMemo(() => {
        ...
        return (
          <div className="flex flex-col gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="animate-pulse bg-white rounded-lg p-6 shadow-sm border border-gray-100"
              >
      ...
      return (
        <div className="space-y-6">
          <div
            className={`space-y-4 transition-all duration-500 ease-in-out ${showAllReviews ? "opacity-100" : "opacity-100"}`}
          >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <h3 className="font-medium capitalize text-[#2D2D2D]">
                    ...
                    <p className="text-[#3B3B3B] text-[15px] leading-relaxed">
                      {r.review}
                    </p>
                  </div>
                </div>
