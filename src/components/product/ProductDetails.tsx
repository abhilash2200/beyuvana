...
const ProductDetails = ({
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="md:text-2xl text-xl leading-tight font-[Grafiels]">
        {name}
      </h1>
      ...
      {certificateGalleryItems.length > 0 && (
        <ImageGalleryDialogWithPdf
          ...
          trigger={
            ...
            >
              <Image
                src={certificateImg || certificateGalleryItems[0]?.image || ""}
                alt="certificate"
                width={40}
                height={40}
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {faq.map((item) => (
              <AccordionItem key={item.id} value={item.id}>
                <AccordionTrigger className="flex justify-between items-center text-[#0C4B33] md:px-5 px-0 py-3 hover:cursor-pointer transition-colors duration-300 no-underline hover:no-underline">
                  <span
                    className={`text-sm ${
                      productId === 1 ? "text-[#0C4B33]" : "text-[#FF4646]"
                    }`}
                  >
                    {item.question}
                  </span>
