...
const InvoiceDisplay = ({
  ...
  const soldBy = sold_by || {
    ...
    address:
      "Mouza Malipanchghara, JL. 17, LR. Dag no. 562, Howrah, Kolkata, West Bengal - 711204",
  ...
  return (
    <div className="invoice-section bg-white p-6 md:p-8 max-w-5xl mx-auto shadow-lg print:shadow-none print:p-4 print:max-w-full print:mx-0">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-3 space-y-1">
              ...
              {soldBy.irn && (
                <p className="text-xs text-gray-700">IRN - {soldBy.irn}</p>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 pb-4 border-b-2 border-gray-300 print:border-black print:grid-cols-3 invoice-section">
        ...
        <div>
          ...
          <div className="space-y-1 text-xs text-gray-700 capitalize">
            ...
            {billTo.business && (
              <p className="text-gray-600">Business : {billTo.business}</p>
            )}
            <p>Address : {billTo.address || "N/A"}</p>
            <p>Phone : {billTo.phone || "N/A"}</p>
            {billTo.gstin && (
              <p className="mt-2 font-medium">GSTIN: {billTo.gstin}</p>
            )}
          </div>
        </div>
      ...
      <div className="mb-6 invoice-section">
        <p className="text-sm font-medium text-gray-700 mb-3 print:text-black">
          Total items: {invoiceSummary.total_items}
        </p>
          ...
          <table className="w-full border-collapse text-xs border border-gray-400 print:border-black">
            <thead>
              <tr className="bg-gray-100 border-b-2 border-gray-400 print:bg-gray-200 print:border-black">
                <th className="text-left p-2 border-r border-gray-400 font-semibold print:border-black print:text-black">
                  Product
                </th>
                <th className="text-left p-2 border-r border-gray-400 font-semibold print:border-black print:text-black">
                  Title
                </th>
                <th className="text-center p-2 border-r border-gray-400 font-semibold print:border-black print:text-black">
                  Qty
                </th>
                <th className="text-right p-2 border-r border-gray-400 font-semibold print:border-black print:text-black">
                  Gross Amount ₹
                </th>
                <th className="text-right p-2 border-r border-gray-400 font-semibold print:border-black print:text-black">
                  Discounts /Coupons ₹
                ...
                <th className="text-right p-2 font-semibold print:text-black">
                  Total ₹
                </th>
              </tr>
            </thead>
            <tbody>
              {invoiceItems.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <td className="p-2 border-r border-gray-400 print:border-black print:text-black">
                      <div className="space-y-1">
                        ...
                        <p className="text-gray-600 text-[10px] print:text-black">
                          CGST:{" "}
                          {item.cgst > 0
                            ? ((item.cgst / item.taxable_value) * 100).toFixed(
                                1,
                              )
                            : "0"}
                          %
                        ...
                        <p className="text-gray-600 text-[10px] print:text-black">
                          ...
                          {item.sgst_utgst > 0
                            ? (
                                (item.sgst_utgst / item.taxable_value) *
                                100
                              ).toFixed(1)
                            : "0"}
                          %
                    ...
                    <td className="text-center p-2 border-r border-gray-400 print:border-black print:text-black">
                      {item.qty}
                    </td>
                    <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                    ...
                    <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                      {item.sgst_utgst.toFixed(2)}
                    </td>
                    <td className="text-right p-2 font-medium print:text-black">
              ...
              <tr className="bg-gray-100 border-t-2 border-gray-400 font-semibold print:bg-gray-200 print:border-black">
                ...
                <td className="text-center p-2 border-r border-gray-400 print:border-black print:text-black">
                  {invoiceSummary.total_items}
                </td>
                <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                  {invoiceSummary.total_amount_before_discount.toFixed(2)}
                </td>
                <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                  -{invoiceSummary.total_discount.toFixed(2)}
                </td>
                <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                  {invoiceSummary.taxable_value.toFixed(2)}
                </td>
                <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                  {invoiceSummary.cgst.toFixed(2)}
                </td>
                <td className="text-right p-2 border-r border-gray-400 print:border-black print:text-black">
                  {invoiceSummary.sgst_utgst.toFixed(2)}
                </td>
                <td className="text-right p-2 print:text-black">
                  {invoiceSummary.grand_total.toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="mt-8 pt-4 border-t border-gray-300">
            <div className="mb-2">
              <p className="text-sm font-semibold text-gray-800">
                {signature ||
                  "Authorized Signatory - BTPL Distribution Private Limited"}
              </p>
            </div>
            <div className="mt-2">
              <Image
