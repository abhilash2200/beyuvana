...
export default function QuantityDropdown({
  ...
  const handleQuantityChange = async (newQuantity: number) => {
    const validQuantity = Math.max(
      ...
      Math.min(CART_CONFIG.MAX_QUANTITY, newQuantity),
    );
  ...
  const handleCustomQuantitySubmit = () => {
    ...
    if (
      quantity >= CART_CONFIG.MIN_QUANTITY &&
  ...
  return (
    <>
      ...
      <Dialog open={isCustomDialogOpen} onOpenChange={setIsCustomDialogOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle className="text-[#057A37] text-center">
              Enter Quantity
            </DialogTitle>
            <DialogDescription>
              Enter a custom quantity ({CART_CONFIG.MIN_QUANTITY}-
          ...
          <DialogFooter className="flex flex-row justify-center gap-2">
            ...
            <Button
              ...
              disabled={
                !customQuantity ||
                parseInt(customQuantity) < CART_CONFIG.MIN_QUANTITY ||
                parseInt(customQuantity) > CART_CONFIG.MAX_QUANTITY
              }
