...
export function SavedAddressesList({
  ...
  if (loading) {
    return (
      <div className="text-center py-4">
        <div
          className={`animate-spin rounded-full ${variant === "mobile" ? "h-6 w-6" : "h-6 w-6"} border-b-2 border-green-600 mx-auto`}
        ></div>
        <p
          className={`text-gray-500 mt-2 ${variant === "mobile" ? "text-xs" : "text-sm"}`}
        >
          Loading addresses...
        </p>
      </div>
  ...
  if (addresses.length === 0) {
    return (
      <div
        className={`text-center ${variant === "mobile" ? "py-6" : "py-6"} text-gray-500`}
      >
        <MapPin
          className={`${variant === "mobile" ? "w-12 h-12" : "w-12 h-12"} mx-auto mb-2 text-gray-300`}
        />
        <p className={variant === "mobile" ? "text-xs" : "text-sm"}>
  ...
  return (
    <div
      className={`space-y-3 ${variant === "desktop" ? "max-h-90 overflow-y-auto" : ""}`}
    >
        ...
        >
          <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-2">
              <h4
                className={`font-medium ${variant === "mobile" ? "text-sm" : "text-[13px]"} text-gray-800 capitalize`}
              >
                {address.fullname}
              </h4>
              {isPrimaryAddress(address) && (
              ...
              {!isPrimaryAddress(address) && (
                ...
                >
                  {settingPrimary === address.id ? (
                    <div
                      className={`animate-spin rounded-full ${variant === "mobile" ? "h-3 w-3" : "h-3 w-3"} border-b border-green-600`}
                    ></div>
                  ) : variant === "mobile" ? (
          ...
          <div
            className={`${variant === "mobile" ? "text-xs" : "text-[12px]"} text-gray-600 space-y-1`}
          >
            <p className="capitalize">{address.address1}</p>
            {address.address2 && (
              <p
                className={`capitalize ${variant === "mobile" ? "" : "text-[#057A37]"}`}
              >
                {variant === "mobile" ? "" : "📍 "}
                {address.address2}
              </p>
            ...
            <div
              className={`flex gap-4 ${variant === "mobile" ? "text-xs" : "text-xs"} text-gray-500`}
            >
