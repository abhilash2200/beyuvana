...
export default function MobileAddAddressSheet({
  open,
  onOpenChange,
  onAddressSaved,
}: MobileAddAddressSheetProps) {
  ...
  const handleSave = async () => {
    ...
    if (!isEditMode && savedAddresses.length >= 3) {
      setError(
        "You can save a maximum of 3 addresses. Please delete an existing address first.",
      );
      return;
    }
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          {!sessionKey && user?.id && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-lg text-sm">
              ...
              <p className="text-xs">
                Please login again to save your address.
              </p>
            </div>
          )}
          {error && (
          ...
          {user?.id && sessionKey && savedAddresses.length > 0 && (
            <div className="bg-white rounded-[5px] border border-green-300">
              ...
              >
                ...
                <div
                  className={`transform transition-transform ${showSavedAddresses ? "rotate-180" : ""}`}
                >
                  <svg
        ...
        <div className="bg-white border-t px-4 py-4 w-full flex gap-3 shrink-0">
          ...
          <Button
            ...
            disabled={
              ...
              !user?.id ||
              !sessionKey ||
              (!isEditMode && savedAddresses.length >= 3)
            }
          >
            {loading
              ? isEditMode
