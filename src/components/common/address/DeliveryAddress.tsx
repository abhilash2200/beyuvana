...
export default function DeliveryAddress({ onAddAddress }: DeliveryAddressProps) {
  ...
  useEffect(() => {
    // LocalStorage se address laana
    const stored =
      typeof window !== "undefined" ? localStorage.getItem("addresses") : null;
        ...
        <>
          ...
          <div className="space-y-3">
            {addresses.map((address) => (
              <label
                key={address.id}
                className="flex items-start gap-3 p-3 bg-white rounded border cursor-pointer"
              >
                <input
                  type="radio"
                  name="address"
