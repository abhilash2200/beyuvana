    ...
    <div>
      {orderDetails && (
        <div className="bg-[#F2F9F3] border border-gray-200 rounded-[20px] p-6 mb-4 space-y-4">
          <h3 className="text-lg font-[Grafiels] text-[#1A2819]">
            Order Information
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              ...
              <p className="font-normal text-black text-[14px]">
                {orderDetails.order_details.order_no}
              </p>
            </div>
            <div>
              <p className="text-sm text-[#1A2819]">Transaction ID</p>
              <p className="font-normal text-black text-[14px]">
                {orderDetails.order_details.txn_id}
              </p>
            </div>
            <div>
              ...
              <p className="font-normal text-black text-[14px]">
                {new Date(
                  orderDetails.order_details.created_date,
                ).toLocaleDateString()}
              </p>
            </div>
            <div>
              ...
              <p
                className={`font-normal text-black text-[14px] ${orderDetails.order_details.pay_status === "SUCCESS" ? "text-green-600" : "text-red-600"}`}
              >
            ...
            <div>
              ...
              <p className="font-normal text-black text-[14px]">
                {orderDetails.order_details.pay_mode}
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="text-[13px]">
            ...
            <div className="flex justify-between items-center py-1">
              ...
              <span className="font-medium">
                ₹{Math.round(order.deliveryPrice)}
              </span>
            </div>
          </div>
          ...
          <div className="text-[13px]">
            <div className="flex justify-between items-center py-1">
              ...
              <span className="font-medium">
                ₹{orderDetails?.order_details?.gross_amount || "0.00"}
              </span>
            </div>
            <div className="flex justify-between items-center py-1">
              <span className="text-gray-700">GST</span>
              <span className="font-medium">
                ₹{orderDetails?.order_details?.gst_amount || "0.00"}
              </span>
            </div>
            <div className="flex justify-between items-center py-1">
              <span className="text-gray-700">Total</span>
              <span className="font-medium">
                ₹{orderDetails?.order_details?.paid_amount || "0.00"}
              </span>
            </div>
            <div className="flex justify-between items-center py-1">
