...
const ProductReview = memo(
  ({ productId, orderStatus, skipOrderCheck = false }: ProductReviewProps) => {
    ...
    const [existingReview, setExistingReview] = useState<ApiReviewItem | null>(
      null,
    );
    ...
    const currentUserReviews = useMemo(() => {
      ...
      return allReviews.filter((review) => {
        ...
        const matchesId =
          ...
          reviewUserId === parseInt(user.id).toString();
    ...
    useEffect(() => {
      const checkReviewEligibility = async () => {
        ...
        try {
          ...
          const eligibility = await reviewApi.canUserReview(
            productId,
            user.id,
            sessionKey,
          );
    ...
    useEffect(() => {
      ...
      setExistingReview(
        currentUserReviews.length > 0 ? currentUserReviews[0] : null,
      );
    }, [currentUserReviews]);
        ...
        <div className="bg-[#F2F9F3] border border-gray-200 p-4 rounded-[20px] transition-all duration-300 ease-in-out">
          {userReviews.length > 0 && (
            <div className="space-y-3 mb-4">
              <h2 className="font-[Grafiels] text-[18px] text-[#1A2819]">
                Your Review:
              </h2>
              {userReviews.map((review, index) => (
                <div
                  key={review.id || index}
                  className="p-3 rounded bg-white border border-gray-200 transition-all duration-200 hover:shadow-md"
                >
                  ...
                  <p className="text-gray-700 capitalize text-sm">
                    {sanitizeTextInput(review.review)}
                  </p>
                  {review.created_at && (
            ...
            <div className="flex gap-1">
              {[...Array(5)].map((_, i) => {
                ...
                return (
                  <FaStar
                    key={i}
                    className={`cursor-pointer text-2xl transition-all duration-150 hover:scale-110 ${
